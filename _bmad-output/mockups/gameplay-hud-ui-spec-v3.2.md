# Gameplay HUD — UI Specification v3.2
...
│   ├── TopBar                           RectTransform (stretch-top, h=186)
...
│   └── BottomBar                        RectTransform (stretch-bottom, h=300)
│       └── ActionPill                   (HLayoutGroup + ContentSizeFitter)
│           ├── UndoButton               (HLayoutGroup, fixed 270×168)
│           │   ├── UndoIcon             TMPro ("↶", 54px white)
│           │   ├── MiniCoin             Image (48×48) + "$" child
│           │   └── CostText             TMPro ("100", 39px bold gold)
│           └── ExtraBottleButton        (HLayoutGroup, fixed 321×168)
...
<!-- Fixed: v3.1 Issue 3 — margin derivation verified: 52px CSS top padding = ~44px status bar + 8px margin -->
### 3.2 `TopBar`

CSS: `.top-bar { padding: 52px 18px 8px 18px }` — the 52px includes ~44px phone status bar + 8px real margin. SafeArea handles the status bar, so TopBar only needs the content-area height.

Derivation: topMargin(8px CSS × 3 = 24px) + tallestElement(gear 46px CSS × 3 = 138px) + bottomMargin(8px CSS × 3 = 24px) = **186px Unity**
...
| Visual | Value | CSS Source |
|---|---|---|
<!-- Patched: Issue 7 — removed "OR" ambiguity, Ideal/Fallback below specifies both options -->
| Image sprite | Baked radial gradient (§6.1) — see Ideal/Fallback below | `radial-gradient(circle at 38% 32%, #ffe866, #f5a623)` |
...
<!-- Patched: Issue 9 — added raycastTarget -->
| raycastTarget | `false` | |
...
| alignment | `Center` | |
<!-- Patched: Issue 10 + Issue 9 — added font + raycastTarget -->
| font | `Nunito-Regular SDF` | |
| raycastTarget | `false` | |
...
| anchorMax | `(1, 1)` | |
| pivot | `(1, 1)` | |
| anchoredPosition | `(-54, -186)` | right: 18×3=54, Y: TopBarHeight(186) |
| default active | `false` | |
...
<!-- Fixed: v3.1 Issue 1 — BottomBar must contain ActionPill, no overflow -->
<!-- Patched: Issue 3A — BottomBar height increased for taller action buttons -->
### 3.7 `BottomBar`

CSS: `.bottom-bar { padding: 0 18px 28px 18px }`

BottomBar height = ActionPill(~216px) + bottomPadding(28px CSS × 3 = 84px) = **300px**
...
<!-- Patched: Issue 6 — anchor changed to bottom-center with CSS bottom padding offset -->
### 3.7a `ActionPill`
...
| anchorMax | `(0.5, 0)` |
| pivot | `(0.5, 0)` |
| anchoredPosition | `(0, 84)` — CSS bottom padding 28px × 3 |
...
<!-- Patched: Issue 1+2+3A — resulting size updated for new button dimensions -->
**Resulting size:** ~`(30 + 270 + 30 + 321 + 30, 24 + 168 + 24)` = **~(681, 216)** — auto-calculated.
...
<!-- Patched: Issue 1 — preferredWidth 210→270, Issue 3A — preferredHeight 114→168 -->
<!-- Patched: Issue 4 — padding L/R corrected from 30→54, Issue 5 — childControl booleans added -->
### 3.7b `UndoButton`
...
|---|---|
| preferredWidth | `270` — padL(54) + icon(54) + gap(15) + coin(48) + gap(15) + cost(~30min) + padR(54) |
| preferredHeight | `168` — matches ExtraBottleButton height for uniform pill appearance |
...
| padding L/R/T/B | `54, 54, 30, 30` | `padding: 10px 18px` × 3 → T/B=30, L/R=54 |
| childForceExpandWidth | `false` | |
| childForceExpandHeight | `false` | |
| childControlWidth | `true` | |
| childControlHeight | `false` | children keep preferred heights |
...
<!-- Patched: Issue 9+10 — added font + raycastTarget -->
...
|---|---|
<!-- Patched: Issue 7 — removed "OR" from MiniCoin, Issue 9+10 — added font/raycast to "$" child -->
| Image | Baked radial gradient (§6.1) — see §3.3b Ideal/Fallback |
| Border ring | 4.5px `#CC8800` (see §3.BORDERS) |
| "$" child TMPro | fontSize `21`, color `#8B6914`, Bold, Center, font `Nunito-Regular SDF`, raycastTarget `false` |
...
<!-- Patched: Issue 2 — preferredWidth 240→321, Issue 3A — preferredHeight 114→168 -->
<!-- Patched: Issue 4 — padding L/R corrected, Issue 5 — childControl booleans added, explicit HLG -->
...
| LayoutElement (in ActionPill) | Value |
|---|---|
| preferredWidth | `321` — padL(54) + bottle(36) + gap(15) + plus(54) + gap(15) + coin(48) + gap(15) + cost(~30min) + padR(54) |
| preferredHeight | `168` — padT(30) + tallestChild(78 bottle body + neck overflow) + padB(30), matched to UndoButton |
...
| childForceExpandHeight | `false` | |
| childControlWidth | `true` | |
| childControlHeight | `false` | children keep preferred heights (BottleIcon=78) |
...
**MiniCoin (child #3):** Same as UndoButton MiniCoin — 48×48, baked radial gradient (§6.1), "$", border ring.
...
<!-- Patched: Issue 8 — Morning LevelText concrete value from ThemeConfig.cs -->
| Level text | `rgba(255,245,230,0.95)` | `rgba(242,235,217,1.0)` — ThemeConfig.TextPrimary = `Color(0.95f, 0.92f, 0.85f)` |
...
<!-- Fixed: v3.1 Issue 2 — size animation explicitly marked optional -->
| Property | Value |
|---|---|
...
| **Size animation (optional polish)** | If implemented: animate `sizeDelta` between `(90,90)` and `(126,126)` in sync with alpha, simulating CSS box-shadow spread growth. If NOT implemented: keep fixed at `(108,108)` from §3.3a — alpha-only pulse is sufficient. |
| Period | `2.5s` |
...
<!-- Patched: Issue 11 — added easing curves -->
| Press 0.92 (60ms, EaseOutQuad) | Release spring 1.05 (90ms, EaseOutBack) → settle 1.0 (60ms, EaseInOutQuad) | Cancel 1.0 (100ms, EaseOutQuad) |
...
<!-- Fixed: v3.1 Issue 1 + 3 — BarHeight and BottomBarHeight updated -->
## 9. Constants Reference (HTML × 3)

```csharp
// Top bar
private const float BarHeight = 186f;             // CSS: 8px margin + 46px gear + 8px margin = 62px × 3 (status bar via SafeArea)
private const float BarMargin = 54f;               // CSS: 18px × 3
private const float BottomBarHeight = 300f;        // ActionPill(~216) + bottomPadding(28px × 3 = 84)
...
<!-- UI Spec Review: 2026-03-25
Reviewer: Nadir
v3.1 Review: 3C 2H 3M 3L → NOT READY
v3.2 Patches: All 11 issues applied (Option A for Issue 3)
v3.2 Re-Review: 0C 0H 1M 2L → quick-fixed all 3
v3.2 Final re-review: 0C 0H 0M 0L — all checklist sections PASS
